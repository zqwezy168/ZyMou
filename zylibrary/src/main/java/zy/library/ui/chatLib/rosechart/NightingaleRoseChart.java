package zy.library.ui.chatLib.rosechart;
...
import zy.library.R;
import zy.library.ui.chatLib.utils.DensityUtil;
import zy.library.ui.chatLib.utils.FontUtil;
import zy.library.ui.chatLib.utils.LogUtil;
...
public class NightingaleRoseChart extends View {
    ...
    private int backColor = getResources().getColor(R.color.gray1);
    ...
    private int rightRectItemW = DensityUtil.dip2px(getContext(), 8);
    private int rightRectItemH = DensityUtil.dip2px(getContext(), 4);
    private int rightRectSpace = DensityUtil.dip2px(getContext(), 4);   //右侧标签上下间距
    private int textSpace = DensityUtil.dip2px(getContext(), 3);

    private int lableTextSize = DensityUtil.dip2px(getContext(), 8);   //右侧标注字体大小
    private int tagTextSize = DensityUtil.dip2px(getContext(), 16);

    private String nullStr = "暂无数据";
    private List<RoseChartBean> dataList;
    private float max;

    private Paint paintArc , paintSelected,painTxt;
    ...
    private final int arrColorRgb[][] = {
            {5, 76, 161},   //    UIColorFromRGB(0xD95F5B),
            {56, 130, 219},     //    UIColorFromRGB(0x7189E6),
            {146, 192, 247}    //    UIColorFromRGB(0x5AB9C7),
...
//            {220, 127, 104}//    UIColorFromRGB(0xDC7F68),
    ...
    public void setLableTextSize(int lableTextSize) {
        this.lableTextSize = DensityUtil.dip2px(getContext(), lableTextSize);          //右边字体
    }


    public void setOutSpace(int outSpace) {
        this.outSpace = DensityUtil.dip2px(getContext(), outSpace);          //图表大小
    }
    ...
    public void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        painTxt.setColor(Color.BLACK);
        painTxt.setTextAlign(Paint.Align.CENTER);
        painTxt.setTextSize(Utils.convertDpToPixel(13f));
...
//            rightLableTop = (getMeasuredHeight()-allHeight)/2;//用于居中
            rightLableTop = -1;
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode == MeasureSpec.EXACTLY) {
            ...
            height = ScrWidth/6 * 5;   //3分之2的位置画图，3分1的位置画标注,设置控件的位置
        }
        int chartWidth = width/6*5;
...
//                LogUtil.e(TAG, "在第"+i+"个lable区域");
    ...
    public void onDraw(Canvas canvas){
        ...
        if(isLoading){
            ...
            paintLabel.setColor(Color.rgb(147, 147, 147));
...
//            LogUtil.i(TAG, "直线坐标：start=("+startX+","+startY +")  end=("+endX+","+endY+")"+"   lineAngle="+lineAngle+"  isRight="+isRight);
            ...
            /**6.绘制占比*/
            textW = FontUtil.getFontlength(paintLabel, (int)bean.getPer()+"");
        ...
        for(int i = 0; i < dataList.size(); i++){
            ...
            if(showChartLable || showChartNum){
                ...
                if(showChartLable){
                    ...
                    /**3、绘制指示标签*/
                    canvas.drawText((int)bean.getPer()+"", bean.getTagTextPoint().x, bean.getTagTextPoint().y, paintLabel);
                }
            }
            ...
            if(showNumTouched && selectedIndex == i) {
                ...
                /**3、绘制数量*/
                canvas.drawText((int)bean.getPer()+"", bean.getTagTextPoint().x, bean.getTagTextPoint().y, paintLabel);
                LogUtil.i(TAG, "绘制bean="+bean.getName()+"   "+(int)bean.getPer());
            }
...
//            LogUtil.i(TAG, "绘制lable矩形"+bean.getColorRect());
            ...
            if(showRightNum){
                /**6.绘制占比*/
                canvas.drawText((int)bean.getPer()+"", bean.getPerTextPoint().x, bean.getPerTextPoint().y , paintLabel);
                /**7、绘制虚线*/
